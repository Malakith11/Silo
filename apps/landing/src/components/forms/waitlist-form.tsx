...
export function WaitlistForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const payload: WaitlistPayload = {
      ...
      interestedModules: moduleOptions.filter(
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit}>
      ...
      <p className="text-sm text-brand-light/80">
        We use these details to curate the right modules and research drops for
        you as the beta expands.
      </p>
      ...
      <fieldset className="space-y-3">
        <legend className="text-sm font-semibold text-brand-light">
      ...
      <p className="text-xs text-brand-light/60">
        By submitting this form you agree to receive email updates about the
        Silo beta and can opt out at any time.
      </p>
    </form>
  );
}
